...
public class Note: NSManagedObject {
    ...
    var imageActual: UIImage? {
        set{
            if newValue == nil {
                ...
                self.image?.imageBig = newValue?.jpegData(compressionQuality: 1.0) as NSData?
                self.imageSmall = newValue?.jpegData(compressionQuality: 0.50) as NSData?
                //UIImageJPEGRepresentation (newValue!, 0.05) as NSData?
            }
            dateUpdate = NSDate()
        }
        get {
    ...
    func addCurrentLocation () {
        LocationManager.shareInstance.getCurrentLocation { (location)
            in
